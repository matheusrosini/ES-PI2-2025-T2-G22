...
import {
  ...
  apiPut,
  apiDelete,
  apiGet,        // usado para instituicoes (GET)
  getInstituicoes // se seu api.js exporta, ótimo; caso não, apiGet('/instituicoes') funciona também
} from "./api.js";
...
// Se o seu api.js EXPORTA getInstituicoes, usa; senão usa apiGet('/instituicoes')
...
function renderDisciplinas(list) {
  ...
  list.forEach(d => {
    // esperamos que a API já retorne o nome da instituição como 'instituicao_nome' ou similar.
    // se não retornar, você pode fazer join no backend ou carregar instituicoes e mapear aqui.
    const instituicaoNome = d.instituicao_nome || d.instituicao_nome_full || d.instituicao?.nome || d.instituicao_nome || "";
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    await addDisciplina(payload); // usa a função do seu api.js
...
tbody.addEventListener("click", async (e) => {
  ...
  if (e.target.classList.contains("btn-edit")) {
    ...
    // Pergunta se quer manter mesma instituição ou trocar
    let novaInstId = null;
    const mudarInst = confirm("Deseja alterar a instituição? (OK = Sim, Cancel = Não)");
    if (mudarInst) {
      // abre uma prompt simples com lista de instituições carregadas no select
